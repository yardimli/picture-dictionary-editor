...
if ( ! $auth_user->is_loggedin() ) {
	...
	if ( $_POST["answer_id"] === "0" ) {
		...
		$answer_lang = $_POST["target_lang"];
		$audio_file = filter_filename( $answer_id  ."_". $answer_lang. "_". time()) . ".mp3";

		if ( $answer !== "" && $answer_lang === "en" ) {
			...
			if ( file_exists( "./audio/story-answer/" . $audio_file ) ) {
				unlink( "./audio/story-answer/" . $audio_file );
			}
			file_put_contents( "./audio/story-answer/" . $audio_file, $resp->getAudioContent() );

			$stmt2 = $story_list->runQuery( 'UPDATE story_answer set audio="' . $audio_file . '" WHERE id=' . $answer_id );
			$stmt2->execute();
			?>
			<script>
				$("#en_audio_player").attr({"src": "../audio/story-answer/<?php echo $audio_file; ?>"});
				$("#en_audio_file").html("<?php echo $audio_file; ?>");
			</script>
			<?php
		}

		if ( $answer !== "" && $answer_lang === "tr" ) {
			$input = new SynthesisInput();
			$input->setText( $answer );
			$voice = new VoiceSelectionParams();
			...
			?>
			<script>
				$("#tr_audio_player").attr({"src": "../audio/story-answer/<?php echo $audio_file; ?>"});
				$("#tr_audio_file").html("<?php echo $audio_file; ?>");
			</script>
			<?php
		}

		if ( $answer !== "" && $answer_lang === "ch" ) {
			$input = new SynthesisInput();
			$input->setText( $answer );
			$voice = new VoiceSelectionParams();
			...
			$resp = $textToSpeechClient->synthesizeSpeech( $input, $voice, $audioConfig );
			if ( file_exists( "./audio/story-answer/" . $audio_file ) ) {
				if ( unlink( "./audio/story-answer/" . $audio_file ) ) {
			...
			$stmt2 = $story_list->runQuery( 'UPDATE story_answer set audio="' . $audio_file . '" WHERE id=' . $answers_id );
			$stmt2->execute();
			?>
			<script>
			$("#ch_audio_player").attr({"src": "../audio/story-answer/<?php echo $audio_file; ?>"});
