...
if ( ! $auth_user->is_loggedin() ) {
	...
	if ( $_POST["question_id"] === "0" ) {
		...
		$question_lang = $_POST["target_lang"];
		$audio_file = filter_filename( $question_id  ."_". $question_lang. "_". time()) . ".mp3";

		if ( $question !== "" && $question_lang === "en" ) {
			...
			if ( file_exists( "./audio/story-question/" . $audio_file ) ) {
				unlink( "./audio/story-question/" . $audio_file );
			}
			file_put_contents( "./audio/story-question/" . $audio_file, $resp->getAudioContent() );

			$stmt2 = $story_list->runQuery( 'UPDATE story_question set audio="' . $audio_file . '" WHERE id=' . $question_id );
			$stmt2->execute();
			?>
			<script>
				$("#en_audio_player").attr({"src": "../audio/story-question/<?php echo $audio_file; ?>"});
				$("#en_audio_file").html("<?php echo $audio_file; ?>");
			</script>
			<?php
		}

		if ( $question !== "" && $question_lang === "tr" ) {
			$input = new SynthesisInput();
			$input->setText( $question );
			$voice = new VoiceSelectionParams();
			...
			?>
			<script>
				$("#tr_audio_player").attr({"src": "../audio/story-question/<?php echo $audio_file; ?>"});
				$("#tr_audio_file").html("<?php echo $audio_file; ?>");
			</script>
			<?php
		}

		if ( $question !== "" && $question_lang === "ch" ) {
			$input = new SynthesisInput();
			$input->setText( $question );
			$voice = new VoiceSelectionParams();
			...
			$resp = $textToSpeechClient->synthesizeSpeech( $input, $voice, $audioConfig );
			if ( file_exists( "./audio/story-question/" . $audio_file ) ) {
				if ( unlink( "./audio/story-question/" . $audio_file ) ) {
			...
			$stmt2 = $story_list->runQuery( 'UPDATE story_question set audio="' . $audio_file . '" WHERE id=' . $questions_id );
			$stmt2->execute();
			?>
			<script>
			$("#ch_audio_player").attr({"src": "../audio/story-question/<?php echo $audio_file; ?>"});
