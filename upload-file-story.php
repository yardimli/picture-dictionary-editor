...
if ( ! $auth_user->is_loggedin() ) {
	...
	if ( $_POST["story_id"] === "0" ) {
		...
		if ( $_POST["upload_type"] === "audio" ) {
			...
			$destination_directory = "./audio/story/";
			$validextensions       = array( "mp3" );
		}

		if ( $_POST["upload_type"] === "picture" ) {
			$max_size              = 500 * 1024; // 500 KB
			$destination_directory = "./pictures/story/";
			...
			if ( in_array( $file_extension, $validextensions ) ) {
				if ( $_FILES["file"]["size"] < ( $max_size ) ) {
					if ( $_FILES["file"]["error"] > 0 ) {
						...
						if ( $_POST["upload_type"] === "picture" ) {
							$stmt2 = $story_list->runQuery( 'UPDATE story set picture="' . $new_filename . '" WHERE id=' . $_POST["story_id"] );
							$stmt2->execute();
						}

						if ( $_POST["upload_type"] === "audio" ) {
							$stmt2 = $story_list->runQuery( 'UPDATE story set audio="' . $new_filename . '" WHERE id=' . $_POST["story_id"] );
							$stmt2->execute();
							?>
							<script>
								$("#audio_player").attr({"src": "../audio/story/<?php echo $new_filename; ?>"});
